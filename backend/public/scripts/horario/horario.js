...
var isAdminValue; // Variable declarada fuera del ámbito de la función

function obtenerTipoUsuario(correo) {
  return axios
    .get("/usertype/" + correo)
    .then(function (response) {
      isAdminValue = response.data; // Asignar el valor a la variable externa
    })
...
function showPop(data) {
  ...
  nEvents.forEach(function (eve, i) {
    eve.addEventListener("click", function () {
      ...
      horario.textContent =
        data[i].startTime + " - " + roundTime(data[i].endTime);
...
function updateEvents(date) {
  ...
  axios
    .get(`http://localhost:8080/reserva/${s}`)
    .then((response) => {
      ...
      if (datosFil.length > 0) {
        ...
        for (var k in datosFil) {
          if (datosFil[k].canceled != 1) {
            ...
            p.textContent =
              datosFil[k].startTime + "-" + roundTime(datos[k].endTime);
...
function cancelbtn(btn, id) {
  btn.addEventListener("click", (event) => {
    ...
    if (confirm("Está seguro de cancelar su reservación?") == true) {
      ...
      axios
        .put(`http://localhost:8080/cancelar/${id}`)
        .then((response) => {
          alert(response.data);
          updateEvents(activeDay);
        })
...
addEventSubmit.addEventListener("click", () => {
  ...
  console.log(
    doubledigit(activeDay) + " " + doubledigit(month + 1) + " " + year
  );
  ...
  axios
    .post("http://localhost:8080/addEvento", {
      start:
        year +
        "-" +
        doubledigit(month + 1) +
